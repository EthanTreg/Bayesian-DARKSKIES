"""
InceptionV4 from https://github.com/zhulf0804/Inceptionv4_and_Inception-ResNetv2.PyTorch
"""
import torch
import torch.nn as nn
from src.utils.units import Conv2d, Reduction_A
...
class Inceptionv4(nn.Module):
    def __init__(self, in_channels=2, classes=4, k=192, l=224, m=256, n=384):
    ...
    def forward(self, x):
        x = self.features(x)
        x = nn.Dropout2d(0.33)(self.global_average_pooling(x))
        x = x.view(x.size(0), -1)
        x = self.linear(x)
        return x