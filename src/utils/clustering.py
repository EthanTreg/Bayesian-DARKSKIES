...
from netloader.utils.types import LossCT
...
class CosineSimilarity(BaseNetwork):
    ...
    def _loss_tensor(self, in_data: Tensor, target: Tensor) -> Tensor:
        """
...
class CompactClusterEncoder(ClusterEncoder):
    ...
    def _loss_function(self, target: Tensor, latent: Tensor, output: Tensor) -> dict[str, Tensor]:
        """
        ...
        ----------
        target : Tensor
            Target labels of shape (N,L) and type float, where N is the batch size and L is the
            number of regression parameters
        latent : Tensor
            Predicted 7D latent values of shape (N,Z) and type float, where Z is the number of
            latent dimensions
        output : Tensor
            Predicted label scores of shape (N,C) and type float, where C is the number of classes
        ...
        dict[str, Tensor]
            ...
            class : Tensor
                Classification loss of shape (1) and type float
            cclp : Tensor
                Compact clustering label propagation loss of shape (1) and type float
            dist : Tensor
                Label regression loss of shape (1) and type float
        """
        loss: dict[str, Tensor] = {'total': torch.tensor(0.).to(self._device)}
        l_idxs: Tensor
        one_hot: Tensor
        bottleneck: Tensor
        ...
        if self.class_loss and l_idxs.any():
            loss['class'] = nn.CrossEntropyLoss()(output[l_idxs], one_hot)
            loss['total'] += self.class_loss * loss['class']

        # Cluster loss
        if self.cluster_loss and l_idxs.any():
            loss['cclp'] = self._label_propagation_cluster_loss(
                torch.cat((latent[l_idxs], latent[~l_idxs])),
                one_hot,
            )
            loss['total'] += self.cluster_loss * loss['cclp']

        # Distance loss
        if self.distance_loss:
            loss['dist'] = self._cluster_centers_loss(latent[l_idxs], target[l_idxs])
            loss['total'] += self.distance_loss * loss['dist']
        return loss

    def _loss_tensor(self, in_data: Tensor, target: Tensor) -> dict[str, Tensor]:
        """
        Loss function of the CompactClusterEncoder.
        ...
        ----------
        in_data : Tensor
            Input data of shape (N,...) and type float, where N is the batch size
        target : Tensor
            Target labels of shape (N,L) and type float, where L is the number of regression
            parameters
    ...
    def saliency(
        ...
        for ids, low_dim, high_dim, *_ in loader:
            ...
            for i in range(latent.size(-1)):
                ...
                self._loss_function(target, latent, output)['total'].backward(retain_graph=True)
